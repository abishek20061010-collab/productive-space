  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      <header className="mb-10">
        <h1 className="font-display text-5xl">Our shop</h1>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-8">
        <button
          onClick={() => nav({ search: {} })}
          className={`px-4 py-2 rounded-full text-sm border ${!params.categorySlug ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}
        >
          All
        </button>
        {cats.categories.map((c) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm border ${params.categorySlug === c.slug ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}
          >
            {c.name}
          </button>
        ))}
        <input
          ...
          onKeyDown={(e) => { if (e.key === "Enter") nav({ search: { ...params, q: (e.target as HTMLInputElement).value || undefined, categorySlug: undefined } }); }}
          className="ml-auto px-4 py-2 rounded-full border border-border bg-background text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-ring"
